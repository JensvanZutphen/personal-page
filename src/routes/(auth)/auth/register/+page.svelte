	...
	let messageType = $derived(
		$message
			? $message.toLowerCase().includes('success') ||
				...
				$message.toLowerCase().includes('welkom') ||
...
<!-- Register form with holographic card -->
{#if !isTouchDevice.current}
	<HolographicCard class="w-full">
		<CardHeader class="space-y-4 pt-6 text-center">
			...
			<CardTitle class="text-2xl font-bold">Account aanmaken</CardTitle>
			<p class="text-muted-foreground">Registreer je om te beginnen met je CRM</p>
		</CardHeader>
		...
		<CardContent>
			...
			<form method="POST" action="?/register" use:enhance class="space-y-4">
				...
				<div class="w-full">
					<Button type="submit" class="mt-6 w-full" size="lg">Account Aanmaken</Button>
				</div>
			</form>
